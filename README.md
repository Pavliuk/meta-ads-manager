...
`meta_ads` з попереднього розділу, кнопками замість CLI/команд. Потребує тих
самих `META_*` змінних у `.env`, що й CLI (без них дії відповідають зрозумілим
попередженням, а не падають).

**`/ads`** відкриває меню:

```
📣 Керування рекламою
[📋 Кампанії]  [➕ Нова кампанія]  [💳 Акаунт]
```

- **💳 Акаунт** → назва, валюта, скільки витрачено загалом, баланс, ліміт витрат
- **📋 Кампанії** → список (🟢 активна / ⏸️ на паузі) → тап на кампанію → картка
  зі статусом, ціллю і бюджетом (у валюті акаунту, напр. `$10.00`), кнопки
  **⏸️ Пауза / ▶️ Запустити**, **🎯 Ad sets**, **📊 Аналітика**,
  **✏️ Редагувати** (назва/бюджет), **📄 Дублювати** (повна копія разом з ad
  set'ами й оголошеннями, PAUSED), **🗑 Видалити** (з підтвердженням)
- **➕ Нова кампанія** → назва → бюджет → **ціль** (Трафік/Взаємодія/Ліди/Охоплення/
  Конверсії/Застосунок, кнопками) → створює кампанію зі статусом PAUSED
- **🎯 Ad sets** (з картки кампанії) → список ad set'ів → тап → пауза/запуск,
  **📋 Оголошення** (список), **➕ Нове оголошення**, **📊 Аналітика**,
  **✏️ Редагувати**, **📄 Дублювати**, **🗑 Видалити**; **➕ Новий ad set**
  запитує назву → бюджет → країни → мінімальний вік → максимальний вік →
  стать (кнопками) → платформи FB/IG/обидві (кнопками) → за бажанням
  **пошук інтересів** для таргетингу (ключове слово → список результатів,
  тап вмикає/вимикає — можна обрати кілька — або пропустити)
- **📋 Оголошення** (з картки ad set'а) → список оголошень → тап → картка зі
  статусом, кнопки **⏸️ Пауза / ▶️ Запустити**, **📊 Аналітика**,
  **✏️ Назва**, **🗑 Видалити**
- **➕ Нове оголошення** → запитує посилання → текст → заголовок → фото (можна
  пропустити кнопкою **⏭ Пропустити**) → створює креатив і оголошення зі
  статусом PAUSED
- **📊 Аналітика** — доступна з картки кампанії, ad set'а і окремого оголошення →
  кнопки періоду (сьогодні/7/30/90 днів) → таблиця витрат/показів/кліків на
  відповідному рівні

Усе так само створюється зі статусом **PAUSED** — кнопка **▶️ Запустити** вмикає
показ свідомо. Видалення (кампанії/ad set'а/оголошення) завжди питає
підтвердження окремою кнопкою і незворотнє. Доступ — лише для `ADMIN_IDS`,
як і `/traffic`.
...
├── ads_commands.py  # /ads — inline-меню + FSM-діалоги створення, обгортка над meta_ads
