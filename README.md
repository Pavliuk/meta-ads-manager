...
- **📋 Кампанії** → список (🟢 активна / ⏸️ на паузі) → тап на кампанію → картка
  ...
  **✏️ Редагувати** (назва/бюджет), **📄 Дублювати** (копія самої кампанії,
  без ad set'ів — Meta відхиляє копіювання оголошень через застаріле поле у
  власному ж механізмі копіювання, тому глибока копія вимкнена), **🗑 Видалити**
  (з підтвердженням)
...
- **🎯 Ad sets** (з картки кампанії) → список ad set'ів → тап → пауза/запуск,
  ...
  **✏️ Редагувати**, **📄 Дублювати** (бюджет/таргетинг, без оголошень —
  та сама причина), **🗑 Видалити**; **➕ Новий ad set**
...
- **📋 Оголошення** (з картки ad set'а) → список оголошень → тап → картка зі
  ...
  **✏️ Назва**, **🗑 Видалити**
- **➕ Нове оголошення** — якщо ad set створений під ціль **🎯 Ліди**, замість
  посилання пропонує обрати чи створити **Instant Form** (форма заявки, що
  відкривається прямо в Facebook/Instagram — ім'я + телефон, без переходу на
  сайт чи бота): назва форми → посилання на політику конфіденційності →
  форма створюється на сторінці. Для решти цілей — як і раніше, запитує
  посилання. Далі однаково: текст → заголовок → фото (можна пропустити
  кнопкою **⏭ Пропустити**) → створює креатив і оголошення зі статусом
  PAUSED
