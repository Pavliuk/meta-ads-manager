...
- **📋 Кампанії** → список (🟢 активна / ⏸️ на паузі) → тап на кампанію → картка
  зі статусом і бюджетом, кнопки **⏸️ Пауза / ▶️ Запустити**, **🎯 Ad sets**,
  **📊 Аналітика**
- **➕ Нова кампанія** → бот запитує назву, потім бюджет (по одному повідомленню
  за раз, з кнопкою **❌ Скасувати** на кожному кроці) → створює кампанію
  зі статусом PAUSED
- **🎯 Ad sets** (з картки кампанії) → список ad set'ів → тап → пауза/запуск,
  **➕ Оголошення**; **➕ Новий ad set** запитує назву → бюджет → країни
- **➕ Оголошення** (з картки ad set'а) → запитує посилання → текст → заголовок →
  фото (можна пропустити кнопкою **⏭ Пропустити**) → створює креатив і
  оголошення зі статусом PAUSED
...
показ свідомо. Доступ — лише для `ADMIN_IDS`, як і `/traffic`.
